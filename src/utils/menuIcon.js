...
/** 自定义菜单图标（对应业务组件，非 Element Plus） */
export const CUSTOM_MENU_ICONS = {
  AI: 'AI',
  Safety: 'Safety',
}

/** 是否为自定义菜单图标 */
export function isCustomMenuIcon(name) {
  return Boolean(name && CUSTOM_MENU_ICONS[name])
}

/** 解析为空心图标名（无映射则原样返回） */
export function resolveMenuIconName(name) {
  if (!name) return name
  if (isCustomMenuIcon(name)) return name
