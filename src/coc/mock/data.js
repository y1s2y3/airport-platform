...
function projectNamePair(index) {
...
export const DANGER_WORK_STATUSES = ['待开工', '作业中', '已完工', '已取消']
...
function buildDangerWorkList(count = 36) {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const permitStatus = status === '已取消' ? '已驳回' : permits[i % 2 === 0 ? 1 : 0]
...
export const DANGER_WORK_RING_SEGMENTS = [
  ...
  { name: '已完工', filter: '已完工', color: '#67c23a' },
  { name: '已取消', filter: '已取消', color: '#f56c6c' },
]
