...
const pageTitle = computed(() => {
  if (isReadonly.value) return '预警配置详情'
  if (route.query.type) return '编辑预警配置'
  return '新增预警配置'
})

const deviceIndicators = {
  '塔吊预警': ['载重(t)', '力矩(tm)', '回转(°)', '高度(m)', '风力(m/s)', '幅度(m)'],
  '升降机预警': ['载重(t)', '人数', '高度(m)'],
  '桩基预警': ['地面高程(m)', '倾角(°)', '深度(m)', '电流(A)'],
  '复合地基预警': ['地面高程(m)', '施工电流(A)', '材料用量(m³)', '深度(m)', '压力(MPa)', '速率(m/min)'],
  '高支模预警': ['沉降(mm)', '位移(mm)', '倾斜(°)'],
  '深基坑预警': ['水平位移(mm)', '竖向位移(mm)', '水位(mm)', '支撑轴力(kN)'],
  '地铁铁路安全监管预警': ['沉降(mm)', '水平位移(mm)', '振动(mm/s)'],
}
...
const deviceOptions = {
  '塔吊预警': ['塔吊QTZ160（#1）', '塔吊QTZ160（#2）', '塔吊QTZ80（#7）'],
  '升降机预警': ['升降机SC200（#2）', '升降机SC200（#4）'],
  '桩基预警': ['桩基钻孔机#5', '桩基旋挖机#8'],
  '复合地基预警': ['复合地基桩机#3', '复合地基桩机#6'],
  '高支模预警': ['高支模监测设备01', '高支模监测设备02'],
  '深基坑预警': ['深基坑监测设备01', '深基坑监测设备02'],
  '地铁铁路安全监管预警': ['地铁监测设备01', '铁路监测设备01'],
}
...
function submitForm() {
  if (!form.alertType) { ElMessage.warning('请选择预警类型'); return }
  if (!form.devices.length) { ElMessage.warning('请选择预警设备'); return }
  ...
  if (!pushForm.remindInterval) { ElMessage.warning('请填写预警未处理提醒时间间隔'); return }
    ...
    <div class="form-body">
      <template v-if="isReadonly">
        <div class="detail-section">
          ...
          <div class="detail-grid">
            <div class="detail-item"><span class="detail-label">预警类型</span><span class="detail-value">{{ form.alertType || '-' }}</span></div>
            <div class="detail-item"><span class="detail-label">预警设备</span><span class="detail-value">{{ form.devices.join('、') || '-' }}</span></div>
        ...
        <div class="detail-section">
          ...
          <el-table :data="thresholdRows" border size="small" style="width:100%;margin-top:12px">
            ...
            <el-table-column prop="indicator" label="预警指标" min-width="140" />
            <el-table-column prop="condition" label="预警条件" width="120" align="center" />
            <el-table-column prop="threshold" label="阈值" width="120" align="center" />
            <el-table-column prop="level" label="预警等级" width="130" align="center" />
          </el-table>
        </div>
        ...
        <div class="detail-section">
          ...
          <div class="detail-grid single">
            <div class="detail-item"><span class="detail-label wide">预警未处理提醒时间间隔</span><span class="detail-value">{{ pushForm.remindInterval }} 分钟</span></div>
          </div>
        </div>
      </template>
      ...
      <el-form v-else :model="form" label-width="100px">
        ...
        <el-row :gutter="16" style="margin-top:12px">
          ...
          <el-col :span="8">
            <el-form-item label="预警设备" required>
        ...
        <el-divider style="margin:20px 0 12px" />
        <div class="section-title">触发条件设置</div>
        <div style="margin-bottom:10px;font-size:12px;color:#999">配置预警指标的触发条件</div>
        <el-table :data="thresholdRows" border size="small" style="width:100%">
          <el-table-column type="index" label="序号" width="50" align="center" />
          <el-table-column min-width="130">
            <template #header><span class="required-column-title">预警指标</span></template>
          ...
          <el-table-column width="110">
            <template #header><span class="required-column-title">预警条件</span></template>
            <template #default="{ row }">
          ...
          <el-table-column width="110">
            <template #header><span class="required-column-title">预警等级</span></template>
            <template #default="{ row }">
        ...
        <div style="margin-top:12px;padding:16px;border:1px solid #eee;border-radius:6px;background:#fafbfc">
          <el-row :gutter="16" style="align-items:center">
            <el-col :span="12">
              <el-form-item label="预警未处理提醒时间间隔" label-width="170px" required style="margin-bottom:0">
