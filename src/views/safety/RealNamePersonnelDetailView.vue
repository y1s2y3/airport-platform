...
function openEducationEdit() {
  const list = detail.value.safetyEducation?.length
    ? detail.value.safetyEducation
    : [createEmptySafetyEducation()]
...
function saveEducation() {
  const invalid = educationDraft.value.some((row) => !row.type || !row.trainDate)
  if (invalid) {
  ...
  try {
    ...
    payload.safetyEducation = educationDraft.value.map((row) => ({
      type: row.type,
    ...
    <div class="detail-body">
      <section class="detail-section">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="工人类型">{{ detail.unit.personnelCategory }}</el-descriptions-item>
      ...
      <section class="detail-section">
        ...
        <el-descriptions :column="2" border>
          <el-descriptions-item label="是否特种作业人员">{{ detail.isSpecial ? '是' : '否' }}</el-descriptions-item>
