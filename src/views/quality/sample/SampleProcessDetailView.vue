  ...
  <div class="qm-page page-card">
    ...
    <template v-else>
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="现场影像资料" :span="2">
          <template v-if="detail.media_files?.length">
            {{
              detail.media_files
                .map((m) => `${m.name}${m.kind === 'video' ? '（视频）' : ''}`)
                .join('、')
            }}
          </template>
          <template v-else-if="detail.photo_files?.length || detail.video_files?.length">
            {{
              [
                ...(detail.photo_files || []),
                ...(detail.video_files || []).map((n) => `${n}（视频）`),
              ].join('、')
            }}
          </template>
          <span v-else>—</span>
        </el-descriptions-item>
        <el-descriptions-item label="文件资料" :span="2">
          {{ detail.doc_files?.length ? detail.doc_files.join('、') : '—' }}
        </el-descriptions-item>
      </el-descriptions>
