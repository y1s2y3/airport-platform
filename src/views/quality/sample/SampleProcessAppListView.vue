...
async function onWithdraw(row) {
  try {
    await ElMessageBox.confirm(`确认撤回 ${row.application_id}？`, '撤回', { type: 'warning' })
  } catch {
    return
  }
  const r = withdrawSample('process', row.application_id)
...
async function onResubmit(row) {
  try {
    await ElMessageBox.confirm(`确认重提 ${row.application_id}？将清空二维码。`, '重提', {
      type: 'warning',
    })
  } catch {
    return
  ...
  <div class="qm-page page-card">
    ...
    <template v-else>
      ...
      <el-table :data="list" stripe border empty-text="暂无工序样板报审">
        ...
        <el-table-column prop="use_part" label="部位" min-width="130" show-overflow-tooltip />
        ...
        <el-table-column label="二维码" width="90">
          <template #default="{ row }">
            <el-tag v-if="row.qr_code" size="small" type="success">已赋码</el-tag>
            <span v-else>—</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              ...
              link
              type="success"
              @click="router.push(`/qm/sample/process/qr?id=${row.application_id}`)"
            >
              二维码
            </el-button>
            <el-button
