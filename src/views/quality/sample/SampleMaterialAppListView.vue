...
function brandLedgerLabel(row) {
  return formatBrandLedgerLabel(row) || '—'
}
</script>
    ...
    <template v-else>
      <div class="filter-bar">
        <el-input
          v-model="keyword"
          clearable
          placeholder="编号 / 样品 / 品牌台账 / 部位"
          style="width: 260px"
          :prefix-icon="Search"
          aria-label="编号 / 样品 / 品牌台账 / 部位"
        />
      ...
      <el-table :data="list" stripe border empty-text="暂无定样审批单">
        ...
        <el-table-column label="品牌台账" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">{{ brandLedgerLabel(row) }}</template>
        </el-table-column>
