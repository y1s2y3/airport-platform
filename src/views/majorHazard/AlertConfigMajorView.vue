...
const editForm = ref({ name: '', hazardType: 'pit', deviceIds: [], indicatorName: '', handler: '', thresholdCondition: '', pushRule: '实时推送', enabled: true })
...
function openAdd() {
  editRule.value = null
  editForm.value = { name: '', hazardType: 'pit', deviceIds: [], indicatorName: '', handler: '', thresholdCondition: '', pushRule: '实时推送', enabled: true }
...
function openEdit(row) {
  ...
  editForm.value = {
    ...
    thresholdCondition: row.thresholdCondition,
...
function handleDelete(row) {
  ElMessageBox.confirm(`确认删除预警规则「${row.name}」？`, '提示', { type: 'warning' }).then(() => {
...
<template>
  <div class="page page-card">
    <div class="page-header">
      <div class="page-breadcrumb">危大工程监测 / 预警配置</div>
      <div class="page-heading">
        <h1 class="page-title">预警配置</h1>
      ...
      <p class="page-tip">配置预警规则，设定监测指标阈值与处置负责人。指标超限时自动生成预警记录并发送站内信。</p>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="editRule ? '编辑预警规则' : '新增预警规则'" width="600px" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="120px">
        <el-form-item label="规则名称" required><el-input v-model="editForm.name" placeholder="如：深基坑水平位移超限预警" /></el-form-item>
