...
function submitInspection() {
  ...
  const rectifyCount = hazardItems.filter(item => item.hasRectify).length
        ...
        <div v-if="form.result === 'hazard'" class="hazard-list">
          <div v-for="(item, idx) in form.hazardItems" :key="idx" class="hazard-item-row">
            ...
            <div class="rectify-section">
              <div class="form-row rectify-switch-row">
                <span class="form-label">下发整改单</span>
                <label class="switch-row">
                  <input type="checkbox" v-model="item.issueRectify" class="switch-input" />
                  <span class="switch-track"><span class="switch-dot"></span></span>
                  <span>{{ item.issueRectify ? '下发' : '不下发' }}</span>
                </label>
              </div>
              <template v-if="item.issueRectify">
                <div class="form-row">
          ...
          <div class="rectify-summary">
            ...
            {{ form.hazardItems.filter(item => item.issueRectify).length }} 份整改单
          </div>
        </div>
      </div>
    </div>
...
.rectify-switch-row { align-items:center; }
