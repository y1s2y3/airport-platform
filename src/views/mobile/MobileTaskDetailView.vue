...
const taskMap = {
  'mt-001': {
    ...
    deadline:'2026-07-28', inspDate:'2026-07-28', status:'待执行', submittedAt:'',
    result:'', normalPhotos:[], hazardItems:[],
  },
  'mt-002': {
    taskNo:'XJ20260720002', planName:'临时用电专项检查', planNo:'JH2026002', planType:'专项巡检',
    project:'T3航站楼扩建工程', executor:'王工', source:'任务推送',
    inspector:'王工', companions:['刘工'],
    deadline:'2026-07-20', inspDate:'2026-07-20', status:'已完成', submittedAt:'2026-07-19 15:30',
    result:'hazard', normalPhotos:[],
    hazardItems:[
      { desc:'五芯电缆破损，线路未按规范敷设', photos:['📷 隐患照片1'], hasRectify:true, rectifyNo:'ZG202607001', rectifyPerson:'张工（安全总监）', rectifyDeadline:'2026-07-30' },
      { desc:'电缆线路沿地明敷未做保护', photos:['📷 隐患照片1'], hasRectify:true, rectifyNo:'ZG202607002', rectifyPerson:'张工（安全总监）', rectifyDeadline:'2026-07-30' },
    ],
  },
  'mt-003': {
    taskNo:'XJ20260721003', planName:'7月第三周安全巡检', planNo:'JH2026001', planType:'周检',
    project:'T3航站楼扩建工程', executor:'王工', source:'任务推送',
    inspector:'王工', companions:['刘工','陈工'],
    deadline:'2026-07-21', inspDate:'2026-07-20', status:'已完成', submittedAt:'2026-07-20 11:20',
...
<template>
  <div class="mp">
    ...
    <div class="sc-card">
      ...
      <div class="sc-row"><span class="sc-lbl">巡检单号</span><span>{{ taskInfo.taskNo }}</span></div>
      ...
      <div class="sc-row"><span class="sc-lbl">单据来源</span><span>{{ taskInfo.source }}</span></div>
      <div class="sc-row"><span class="sc-lbl">巡检人</span><span>{{ taskInfo.inspector }}</span></div>
    ...
    <div class="sc-card">
      ...
      <div class="sc-row">
        ...
        <span :style="{color:taskInfo.result==='hazard'?'#e53935':'#34a853',fontWeight:600}">
          {{ taskInfo.result==='hazard'?'⚠ 有隐患（'+taskInfo.hazardItems.length+'项）':'✓ 全部正常' }}
        </span>
      </div>
    ...
    <div v-if="taskInfo.result==='hazard'" class="hazard-area">
      ...
      <div v-for="(hi,i) in taskInfo.hazardItems" :key="i" class="hazard-card">
        ...
        <div v-if="hi.hasRectify" class="rectify-card">
          ...
          <div class="sc-row"><span class="sc-lbl">整改单号</span><span class="rectify-no">{{ hi.rectifyNo }}</span></div>
          <div class="sc-row"><span class="sc-lbl">整改人</span><span>{{ hi.rectifyPerson }}</span></div>
          <div class="sc-row"><span class="sc-lbl">整改截止</span><span>{{ hi.rectifyDeadline }}</span></div>
        </div>
      </div>
    </div>
    ...
    <div class="bottom-bar">
      <div v-if="taskInfo.status==='已完成'&&taskInfo.result==='hazard'" class="bottom-info warn">
        ⚠ 已发现 {{ taskInfo.hazardItems.length }} 项隐患，已下发 {{ taskInfo.hazardItems.filter(h=>h.hasRectify).length }} 份整改单
      </div>
...
.rectify-card { background:#fafafa; border-radius:8px; padding:10px 12px; margin-top:8px; border-left:3px solid #8f0045; }
.rectify-title { font-size:12px; font-weight:600; color:#8f0045; margin-bottom:6px; }
.rectify-no { color:#4285f4; font-weight:500; }
...
.bottom-info { padding:8px 14px; border-radius:8px; text-align:center; font-size:13px; }
.bottom-info.warn { background:#fff8e6; color:#e53935; font-weight:500; }
.bottom-info.ok { background:#e8f5e9; color:#34a853; font-weight:500; }
</style>
