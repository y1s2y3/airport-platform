...
import { ensureInspectionOverdueReminders, inspectionOverdueReminders } from '../../mock/personalCenter.js'
...
const reminderMessages = computed(() => [...inspectionOverdueReminders])
...
<template>
  <div class="message-page">
    ...
    <section class="center-switcher">
      <button
        v-for="item in [{name:'流程中心',icon:'⇄'},{name:'消息提醒',icon:'◷'},{name:'预警中心',icon:'⚠'}]"
        :key="item.name"
      ...
      >
        ...
        <i v-else-if="item.name === '消息提醒' && reminderMessages.length">{{ reminderMessages.length }}</i>
      </button>
    </section>
    ...
    <template v-else-if="activeCenter === '消息提醒'">
      <main class="message-list">
        <article v-for="item in reminderMessages" :key="item.id" class="message-card reminder-card">
          <div class="message-card-head">
            <strong>{{ item.title }}</strong>
            <span class="status-tag todo">{{ item.readStatus }}</span>
          </div>
          <div class="message-field"><span>所属模块：</span><b>{{ item.module }}</b></div>
          <div class="message-field notice-content"><span>提醒内容：</span><b>{{ item.content }}</b></div>
          <div class="message-field"><span>提醒时间：</span><b>{{ item.time }}</b></div>
        </article>
        <div v-if="!reminderMessages.length" class="empty-state">暂无新提醒</div>
      </main>
    </template>
