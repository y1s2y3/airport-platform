...
const props = defineProps({
  ...
  title: { type: String, default: '个人中心' },
  /** 标签切换时同步到路由的 base path */
  basePath: { type: String, default: '/personal-center' },
})
...
function onTabChange(name) {
  ...
  noticeSelection.value = []
  warningSelection.value = []
  router.replace({ path: props.basePath, query: name === 'todo' ? {} : { tab: name } })
}
...
<template>
  <div class="pc-page" :class="{ 'pc-page--embedded': !showTitle }">
    <h1 v-if="showTitle" class="pc-title">{{ title }}</h1>
