...
export const nodeArchiveDocConfigs = reactive([
  ...
  { node_type: 9, docs: ['实体工程验收汇总报告'] },
  { node_type: 10, docs: ['专项验收汇总报告'] },
])
...
/**
 * 任务审批链（D2）：已登记 → 用快照；未登记 → 档案侧按类型的当前链
 * owner_final_required 的业主/总监终审为本地规则，叠加在快照之后
 */
export function getArchiveChain(task) {
  const sync = getArchiveSync(task.id)
  const base = sync
    ? [...sync.chain_snapshot]
    : [...(ARCHIVE_CHAIN_BY_TYPE[task.task_type] || ['监理'])]
  if (task.task_type === 1 && task.owner_final_required === 1 && !base.includes('业主/总监')) {
    base.push('业主/总监')
  }
  return base
}
...
/** C2 拦截判定：节点配置了需填报档案文件 && 未登记 → 拦截提交/通过 */
...
export function registerArchiveDoc(task, { operator_id = 'u-sg-01', sync_source = 'manual' } = {}) {
  ...
  const chain = [...(ARCHIVE_CHAIN_BY_TYPE[task.task_type] || ['监理'])]
