/**
 * 质量验评 Mock — 字段名/枚举严格对齐 data-model-for-验评 V2.3.1
 * 禁止臆造字段；展示用项目名称通过 project_id 解析，不写入任务实体。
 * V2.3.1 新增：is_draft / archive_instance_id（Q12 一任务一档案文档）、容器任务 9/10、
 * 整改单 status_changed_at + archive_doc_status（§4.7、D4）、TASK_LINK_MATERIAL / TASK_LINK_SAMPLE
 */
import { reactive } from 'vue'
import { COC_PROJECT_OPTIONS } from '../config/projectOptions.js'
import { getEffectiveSpecialties } from '../constants/wbsSpecialty.js'
import { nowStr } from '../utils/datetime.js'

/** 任务 status：0待提交 1审批中 2已通过 3已驳回（与品牌报审口径一致；旧整改/复验主路径已废止） */
export const TASK_STATUS = {
  0: '待提交',
  1: '审批中',
  2: '已通过',
  3: '已驳回',
}

/** 列表状态筛选项 */
export const TASK_STATUS_FILTER_OPTIONS = Object.entries(TASK_STATUS).map(([value, label]) => ({
  value,
  label,
}))

/** 列表「当前状态」展示 */
export function getTaskDisplayStatus(task) {
  if (!task) return { key: '', label: '—', tagType: 'info' }
  const s = Number(task.status)
  const tagType = s === 0 ? 'info' : s === 1 ? 'warning' : s === 2 ? 'success' : s === 3 ? 'danger' : 'info'
  return {
    key: String(task.status),
    label: TASK_STATUS[task.status] || '—',
    tagType,
  }
}

/** 计划 status：待复核 / 未开始 / 进行中 / 已完成 */
export const PLAN_STATUS = {
  0: '待复核',
  1: '未开始',
  2: '进行中',
  3: '已完成',
}

/** 整改单 status */
export const RECTIFY_STATUS = {
  0: '待整改',
  1: '整改中',
  2: '待复验',
  3: '已关闭',
  4: '逾期未关闭',
}

/**
 * 验评目录树节点类型
 * 树骨架：8 竣工验收(根) → 9 实体工程验收(分类) / 10 专项验收(分类) → 单位~检验批 / 专项节点
 * 说明：node_type=9、10 仅作目录分类，不做验收；业务上单位工程上一级视为项目层级。
 */
export const WBS_TREE_NODE_TYPE_LABEL = {
  1: '单位工程',
  2: '子单位工程',
  3: '分部',
  4: '子分部',
  5: '分项',
  6: '检验批',
  7: '专项节点',
  8: '竣工验收',
  9: '实体工程验收',
  10: '专项验收',
}

export const NODE_TYPE_LABEL = { ...WBS_TREE_NODE_TYPE_LABEL }

/** 用户可维护的业务节点（不含系统骨架 8/9/10） */
export const WBS_EDITABLE_NODE_TYPES = [1, 2, 3, 4, 5, 6, 7]

/** 系统固定骨架节点：根竣工 + 实体/专项二级容器，不可删除 */
export const WBS_SYSTEM_NODE_TYPES = [8, 9, 10]

/** 目录树展示的全部节点类型 */
export const WBS_TREE_NODE_TYPES = Object.keys(WBS_TREE_NODE_TYPE_LABEL).map(Number)

/** 实体验收链：单位工程 → … → 检验批 */
export const WBS_ENTITY_NODE_TYPES = [1, 2, 3, 4, 5, 6]

export const TASK_TYPE_LABEL = {
  1: '检验批验收',
  2: '分项工程验收',
  3: '子分部工程验收',
  4: '分部工程验收',
  5: '单位工程验收',
  6: '专项验收',
  7: '竣工验收',
  8: '子单位工程验收',
  /** 历史类型：实体/专项分类节点已不可发起，仅兼容旧数据展示 */
  9: '实体验收汇总',
  10: '专项验收汇总',
}

/** node_type → task_type（分类节点 9/10 无有效创建映射，创建接口会拦截） */
export const NODE_TO_TASK_TYPE = {
  6: 1,
  5: 2,
  4: 3,
  3: 4,
  2: 8,
  1: 5,
  7: 6,
  8: 7,
  // 9、10 仅为分类，不映射可创建任务类型
}

export function resolveProjectName(project_id) {
  const hit = COC_PROJECT_OPTIONS.find((p) => p.id === project_id)
  return hit?.fullName || hit?.label || project_id || '—'
}

export { nowStr }

const pid = 'p-000'

/** WBS_NODE — 骨架：竣工验收 → 实体工程验收 / 专项验收 → 业务节点 */
export const wbsNodes = reactive([
  {
    id: 'wn-complete-1',
    project_id: pid,
    parent_id: '',
    node_type: 8,
    node_name: '项目竣工验收',
    location_code: 'T2',
    batch_type_id: '',
    form_template_ids: ['ft-complete'],
    specialties: [],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 0,
    created_by: 'hq_admin',
    created_at: '2026-06-01 08:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 08:00:00',
  },
  {
    id: 'wn-entity-root',
    project_id: pid,
    parent_id: 'wn-complete-1',
    node_type: 9,
    node_name: '实体工程验收',
    location_code: '',
    batch_type_id: '',
    form_template_ids: [],
    specialties: [],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 08:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 08:00:00',
  },
  {
    id: 'wn-special-root',
    project_id: pid,
    parent_id: 'wn-complete-1',
    node_type: 10,
    node_name: '专项验收',
    location_code: '',
    batch_type_id: '',
    form_template_ids: [],
    specialties: [],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 2,
    created_by: 'hq_admin',
    created_at: '2026-06-01 08:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 08:00:00',
  },
  {
    id: 'wn-unit-1',
    project_id: pid,
    parent_id: 'wn-entity-root',
    node_type: 1,
    node_name: 'T2航站楼主体单位工程',
    location_code: 'T2-主体',
    batch_type_id: '',
    form_template_ids: ['ft-unit-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级分部未全部通过，单位工程不可已完成
    accept_status: 0,
    batch_scheme_id: 'bs-001',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-20 18:00:00',
  },
  {
    id: 'wn-unit-2',
    project_id: pid,
    parent_id: 'wn-entity-root',
    node_type: 1,
    node_name: 'T2航站楼机电单位工程',
    location_code: 'T2-机电',
    batch_type_id: '',
    form_template_ids: ['ft-unit-record'],
    specialties: ['机电'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级子单位已通过，本单位工程审批中（与 tk-unit-2 一致）
    accept_status: 1,
    batch_scheme_id: '',
    sort_no: 2,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:02:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-20 18:00:00',
  },
  {
    id: 'wn-unit-3',
    project_id: pid,
    parent_id: 'wn-entity-root',
    node_type: 1,
    node_name: '空侧捷运线单位工程',
    location_code: 'AGT',
    batch_type_id: '',
    form_template_ids: ['ft-unit-record'],
    specialties: ['市政'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级分部未通过，不可已完成
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 3,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:04:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-20 18:00:00',
  },
  {
    id: 'wn-unit-4',
    project_id: pid,
    parent_id: 'wn-entity-root',
    node_type: 1,
    node_name: '飞行区下穿通道单位工程',
    location_code: 'TUN',
    batch_type_id: '',
    form_template_ids: ['ft-unit-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级分部/分项未齐，不可已完成
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 4,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:06:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:06:00',
  },
  {
    id: 'wn-subunit-2',
    project_id: pid,
    parent_id: 'wn-unit-2',
    node_type: 2,
    node_name: '强电系统子单位工程',
    location_code: 'T2-机电-强电',
    batch_type_id: '',
    form_template_ids: [],
    specialties: ['机电'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级分部已全部通过
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:08:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:08:00',
  },
  {
    id: 'wn-div-2',
    project_id: pid,
    parent_id: 'wn-subunit-2',
    node_type: 3,
    node_name: '供配电分部',
    location_code: 'T2-机电-配电',
    batch_type_id: '',
    form_template_ids: ['ft-div-record'],
    specialties: ['机电'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:09:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:09:00',
  },
  {
    id: 'wn-item-2',
    project_id: pid,
    parent_id: 'wn-div-2',
    node_type: 5,
    node_name: '电缆敷设分项',
    location_code: 'T2-机电-电缆',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['机电'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:09:30',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:09:30',
  },
  {
    id: 'wn-batch-me-1',
    project_id: pid,
    parent_id: 'wn-item-2',
    node_type: 6,
    node_name: '低压电缆检验批-B1区',
    location_code: 'T2-B1-电缆',
    batch_type_id: 'bt-concrete',
    form_template_ids: [],
    specialties: ['机电'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:09:40',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:09:40',
  },
  {
    id: 'wn-div-3',
    project_id: pid,
    parent_id: 'wn-unit-3',
    node_type: 3,
    node_name: '轨道梁分部',
    location_code: 'AGT-梁',
    batch_type_id: '',
    form_template_ids: ['ft-div-record'],
    specialties: ['市政'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:05:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:05:00',
  },
  {
    id: 'wn-subdiv-3',
    project_id: pid,
    parent_id: 'wn-div-3',
    node_type: 4,
    node_name: '预制梁子分部',
    location_code: 'AGT-预制梁',
    batch_type_id: '',
    form_template_ids: ['ft-subdiv-custom'],
    specialties: ['市政'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:05:20',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:05:20',
  },
  {
    id: 'wn-item-3',
    project_id: pid,
    parent_id: 'wn-subdiv-3',
    node_type: 5,
    node_name: '混凝土分项',
    location_code: 'AGT-砼',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['市政'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:05:40',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:05:40',
  },
  {
    id: 'wn-batch-agt-1',
    project_id: pid,
    parent_id: 'wn-item-3',
    node_type: 6,
    node_name: '轨道梁混凝土检验批-DK0+100',
    location_code: 'AGT-DK0+100',
    batch_type_id: 'bt-concrete',
    form_template_ids: [],
    specialties: ['市政'],
    is_hidden_work: 0,
    is_critical: 1,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:05:50',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:05:50',
  },
  {
    id: 'wn-div-4',
    project_id: pid,
    parent_id: 'wn-unit-4',
    node_type: 3,
    node_name: '结构主体分部',
    location_code: 'TUN-主体',
    batch_type_id: '',
    form_template_ids: ['ft-div-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级分项未通过，分部不可已完成
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:07:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:07:00',
  },
  {
    id: 'wn-item-4',
    project_id: pid,
    parent_id: 'wn-div-4',
    node_type: 5,
    node_name: '防水分项',
    location_code: 'TUN-防水',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['结构'],
    is_hidden_work: 1,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:07:20',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:07:20',
  },
  {
    id: 'wn-unit-p001',
    project_id: 'p-001',
    parent_id: '',
    node_type: 1,
    node_name: '三跑道道面单位工程',
    location_code: 'RWY3',
    batch_type_id: '',
    form_template_ids: ['ft-unit-record'],
    specialties: ['道面'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-05 08:50:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-05 08:50:00',
  },
  {
    id: 'wn-div-1',
    project_id: pid,
    parent_id: 'wn-unit-1',
    node_type: 3,
    node_name: '主体结构分部',
    location_code: 'T2-主体',
    batch_type_id: '',
    form_template_ids: ['ft-div-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:10:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-01 09:10:00',
  },
  {
    id: 'wn-item-1',
    project_id: pid,
    parent_id: 'wn-div-1',
    node_type: 5,
    node_name: '钢筋分项工程',
    location_code: 'T2-F3',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级检验批未全部通过，分项不可进入报审/完成
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:20:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-10 10:00:00',
  },
  {
    id: 'wn-batch-1',
    project_id: pid,
    parent_id: 'wn-item-1',
    node_type: 6,
    node_name: '三层梁板钢筋检验批-轴1~5',
    location_code: 'T2-F3-A区',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 1,
    is_critical: 1,
    accept_status: 1,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-02 10:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-15 11:00:00',
  },
  {
    id: 'wn-batch-2',
    project_id: pid,
    parent_id: 'wn-item-1',
    node_type: 6,
    node_name: '三层梁板钢筋检验批-轴6~10',
    location_code: 'T2-F3-B区',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 1,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 2,
    created_by: 'hq_admin',
    created_at: '2026-06-02 10:05:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-02 10:05:00',
  },
  {
    id: 'wn-batch-3',
    project_id: pid,
    parent_id: 'wn-item-1',
    node_type: 6,
    node_name: '柱钢筋检验批-C区',
    location_code: 'T2-F3-C区',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 1,
    is_critical: 0,
    accept_status: 3,
    batch_scheme_id: '',
    sort_no: 3,
    created_by: 'hq_admin',
    created_at: '2026-06-03 09:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-12 16:00:00',
  },
  {
    id: 'wn-item-p001',
    project_id: 'p-001',
    parent_id: 'wn-unit-p001',
    node_type: 5,
    node_name: '水泥混凝土面层分项',
    location_code: 'RWY3-面层',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['道面'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-05 08:55:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-05 08:55:00',
  },
  {
    id: 'wn-batch-4',
    project_id: 'p-001',
    parent_id: 'wn-item-p001',
    node_type: 6,
    node_name: '三跑道道面混凝土检验批-K3+100',
    location_code: 'RWY3-K3',
    batch_type_id: 'bt-concrete',
    form_template_ids: [],
    specialties: ['道面'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-05 09:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-05 09:00:00',
  },
  {
    id: 'wn-batch-5',
    project_id: pid,
    parent_id: 'wn-item-1',
    node_type: 6,
    node_name: '板面钢筋检验批-D区',
    location_code: 'T2-F3-D区',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 3,
    batch_scheme_id: '',
    sort_no: 4,
    created_by: 'hq_admin',
    created_at: '2026-06-04 09:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-14 09:00:00',
  },
  {
    id: 'wn-batch-hist',
    project_id: pid,
    parent_id: 'wn-item-1',
    node_type: 6,
    node_name: '二层梁板钢筋检验批（已办结）',
    location_code: 'T2-F2-A区',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 1,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 9,
    created_by: 'hq_admin',
    created_at: '2026-05-01 09:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-06-21 15:00:00',
  },
  {
    id: 'wn-item-ready',
    project_id: pid,
    parent_id: 'wn-div-1',
    node_type: 5,
    node_name: '模板分项工程（可报验样例）',
    location_code: 'T2-F3-模板',
    batch_type_id: '',
    form_template_ids: ['ft-item-record'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    // 下级检验批已全部通过，本分项审批中（与 tk-007 一致）
    accept_status: 1,
    batch_scheme_id: '',
    sort_no: 2,
    created_by: 'hq_admin',
    created_at: '2026-06-01 09:25:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-16 10:00:00',
  },
  {
    id: 'wn-batch-ready-1',
    project_id: pid,
    parent_id: 'wn-item-ready',
    node_type: 6,
    node_name: '模板安装检验批（已通过）',
    location_code: 'T2-F3-模板-1',
    batch_type_id: 'bt-rebar',
    form_template_ids: [],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-02 11:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-16 09:00:00',
  },
  {
    id: 'wn-special-fire',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '消防验收',
    location_code: 'T2',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['消防'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 1,
    created_by: 'hq_admin',
    created_at: '2026-06-01 10:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-19 16:00:00',
  },
  {
    id: 'wn-special-cd',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '人防验收',
    location_code: 'T2-B1',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['人防'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 2,
    created_by: 'hq_admin',
    created_at: '2026-06-01 10:10:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-14 15:00:00',
  },
  {
    id: 'wn-special-energy',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '节能验收',
    location_code: 'T2',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['节能'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 3,
    created_by: 'hq_admin',
    created_at: '2026-06-01 10:20:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-13 11:00:00',
  },
  {
    id: 'wn-special-planning',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '规划验收',
    location_code: 'T2',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['结构'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 4,
    created_by: 'hq_admin',
    created_at: '2026-06-01 10:30:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-20 09:00:00',
  },
  {
    id: 'wn-special-equip',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '特种设备验收',
    location_code: 'T2-电梯',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['消防'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 2,
    batch_scheme_id: '',
    sort_no: 5,
    created_by: 'hq_admin',
    created_at: '2026-06-01 10:40:00',
    updated_by: 'hq_admin',
    updated_at: '2026-07-21 10:00:00',
  },
  // 以下专项叶子仅挂「待提交」演示单；exclude_from_complete_gate 避免打断竣工前置门禁
  {
    id: 'wn-special-fire-west',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '消防验收（西翼）',
    location_code: 'T2-西翼',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['消防'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 6,
    exclude_from_complete_gate: 1,
    created_by: 'hq_admin',
    created_at: '2026-08-01 10:00:00',
    updated_by: 'hq_admin',
    updated_at: '2026-08-01 10:00:00',
  },
  {
    id: 'wn-special-cd-phase2',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '人防验收（二期）',
    location_code: 'T2-B2人防',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['人防'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 7,
    exclude_from_complete_gate: 1,
    created_by: 'hq_admin',
    created_at: '2026-08-01 10:05:00',
    updated_by: 'hq_admin',
    updated_at: '2026-08-01 10:05:00',
  },
  {
    id: 'wn-special-energy-sample',
    project_id: pid,
    parent_id: 'wn-special-root',
    node_type: 7,
    node_name: '节能验收（样板区）',
    location_code: 'T2-样板区',
    batch_type_id: '',
    form_template_ids: ['ft-special-fire'],
    specialties: ['节能'],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 8,
    exclude_from_complete_gate: 1,
    created_by: 'hq_admin',
    created_at: '2026-08-01 10:10:00',
    updated_by: 'hq_admin',
    updated_at: '2026-08-01 10:10:00',
  },
])

/** BATCH_TYPE 简表 */
export const batchTypes = reactive([
  {
    id: 'bt-rebar',
    type_code: 'REBAR',
    type_name: '钢筋工程检验批',
    specialties: ['结构'],
    status: 1,
    remark: '',
  },
  {
    id: 'bt-concrete',
    type_code: 'CONCRETE',
    type_name: '混凝土工程检验批',
    specialties: ['结构'],
    status: 1,
    remark: '',
  },
])

/** 检验批类型枚举展示 */
export const BATCH_TYPE_LABEL = Object.fromEntries(
  batchTypes.map((t) => [t.id, t.type_name]),
)

export function resolveBatchTypeName(batch_type_id) {
  if (!batch_type_id) return '—'
  return BATCH_TYPE_LABEL[batch_type_id] || batch_type_id
}

/** ACCEPTANCE_PLAN */
export const acceptancePlans = reactive([
  {
    id: 'pl-001',
    plan_no: 'JH-2026-001',
    project_id: pid,
    plan_type: 1,
    plan_name: 'T2三层梁板钢筋验收计划',
    wbs_node_id: 'wn-item-1',
    content: '完成三层梁板钢筋检验批验收',
    plan_date: '2026-07-31',
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    status: 2,
    change_flag: 0,
    reviewer_id: 'u-jl-01',
    review_time: '2026-06-10 14:00:00',
    review_opinion: '',
    remark: '',
  },
  {
    id: 'pl-002',
    plan_no: 'JH-2026-002',
    project_id: pid,
    plan_type: 1,
    plan_name: '柱钢筋验收计划',
    wbs_node_id: 'wn-batch-3',
    content: '柱钢筋检验批',
    plan_date: '2026-07-20',
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    status: 2,
    change_flag: 0,
    reviewer_id: 'u-jl-01',
    review_time: '2026-06-12 10:00:00',
    review_opinion: '',
    remark: '',
  },
  {
    id: 'pl-003',
    plan_no: 'JH-2026-003',
    project_id: pid,
    plan_type: 1,
    plan_name: '板面钢筋验收计划',
    wbs_node_id: 'wn-batch-5',
    content: '板面钢筋',
    plan_date: '2026-08-15',
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    status: 1,
    change_flag: 0,
    reviewer_id: 'u-jl-01',
    review_time: '2026-06-15 09:00:00',
    review_opinion: '',
    remark: '',
  },
  {
    id: 'pl-004',
    plan_no: 'JH-2026-004',
    project_id: 'p-001',
    plan_type: 1,
    plan_name: '三跑道道面混凝土验收计划',
    wbs_node_id: 'wn-batch-4',
    content: '道面混凝土检验批',
    plan_date: '2026-08-30',
    contractor_org_id: 'org-sg-02',
    supervisor_org_id: 'org-jl-02',
    status: 1,
    change_flag: 0,
    reviewer_id: 'u-jl-02',
    review_time: '2026-06-20 11:00:00',
    review_opinion: '',
    remark: '',
  },
  {
    id: 'pl-005',
    plan_no: 'JH-2026-005',
    project_id: pid,
    plan_type: 1,
    plan_name: '轴6~10钢筋验收计划',
    wbs_node_id: 'wn-batch-2',
    content: '轴6~10梁板钢筋',
    plan_date: '2026-07-25',
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    status: 0,
    change_flag: 0,
    reviewer_id: '',
    review_time: '',
    review_opinion: '',
    remark: '',
  },
])

/**
 * INSPECTION_TASK — 列表关键展示：task_no、project_id(→项目名称)、status
 * ≥5 条，覆盖多状态
 */
export const inspectionTasks = reactive([
  {
    id: 'tk-001',
    task_no: 'YS-2026-001',
    project_id: pid,
    wbs_node_id: 'wn-batch-1',
    plan_id: 'pl-001',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F3-A区 轴1~5',
    location_id: 'loc-rebar-a1',
    location_ids: ['loc-rebar-a1', 'loc-rebar-a2'],
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 1,
    result: 0,
    is_hidden_work: 1,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-15 10:30:00',
    reviewer_id: 'u-jl-01',
    finish_time: '',
    remark: '',
    created_by: 'u-sg-01',
    created_at: '2026-07-14 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-15 10:30:00',
  },
  {
    id: 'tk-002',
    task_no: 'YS-2026-002',
    project_id: pid,
    wbs_node_id: 'wn-batch-2',
    plan_id: 'pl-005',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F3-B区 轴6~10',
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 0,
    result: 0,
    is_hidden_work: 1,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    remark: '',
    created_by: 'u-sg-01',
    created_at: '2026-07-10 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-10 09:00:00',
  },
  {
    id: 'tk-003',
    task_no: 'YS-2026-003',
    project_id: pid,
    wbs_node_id: 'wn-batch-3',
    plan_id: 'pl-002',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F3-C区 柱钢筋',
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 3,
    result: 2,
    is_hidden_work: 1,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-11 14:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-12 17:00:00',
    remark: '已驳回存档，可重新申报（原整改路径已废止）',
    created_by: 'u-sg-01',
    created_at: '2026-07-10 11:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-12 17:00:00',
  },
  {
    id: 'tk-004',
    task_no: 'YS-2026-004',
    project_id: pid,
    wbs_node_id: 'wn-batch-5',
    plan_id: 'pl-003',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F3-D区',
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-08 09:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-13 10:00:00',
    remark: '已驳回存档，可重新申报（原整改路径已废止）',
    created_by: 'u-sg-01',
    created_at: '2026-07-07 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-13 10:00:00',
  },
  {
    id: 'tk-005',
    task_no: 'YS-2026-005',
    project_id: 'p-001',
    wbs_node_id: 'wn-batch-4',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 1,
    specialties: ['道面'],
    location_name: '三跑道 K3+100',
    form_template_id: 'ft-batch-concrete',
    form_data: {},
    batch_type_id: 'bt-concrete',
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-02',
    supervisor_org_id: 'org-jl-02',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    remark: '临时抽查，不挂计划建任务',
    created_by: 'u-sg-02',
    created_at: '2026-07-16 08:30:00',
    updated_by: 'u-sg-02',
    updated_at: '2026-07-16 08:30:00',
  },
  {
    id: 'tk-006',
    task_no: 'YS-2026-006',
    project_id: pid,
    wbs_node_id: 'wn-batch-hist',
    plan_id: 'pl-001',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F2-A区（历史已办结样例）',
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 2,
    result: 1,
    is_hidden_work: 1,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-06-20 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-06-21 15:00:00',
    remark: '',
    created_by: 'u-sg-01',
    created_at: '2026-06-19 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-06-21 15:00:00',
  },
  {
    id: 'tk-007',
    task_no: 'YS-2026-007',
    project_id: pid,
    wbs_node_id: 'wn-item-ready',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 2,
    specialties: ['结构'],
    location_name: 'T2-F3-模板',
    form_template_id: 'ft-item-record',
    form_data: { 'ft-item-record': { 分项工程名称: '模板分项工程', 检验批数量: '1' } },
    batch_type_id: '',
    status: 1,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-17 09:10:00',
    reviewer_id: 'u-jl-01',
    finish_time: '',
    remark: '分项已报验，个人中心待审批演示',
    created_by: 'u-sg-01',
    created_at: '2026-07-17 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-17 09:10:00',
  },
  {
    id: 'tk-008',
    task_no: 'ZX-2026-001',
    task_name: '消防专项验收',
    project_id: pid,
    wbs_node_id: 'wn-special-fire',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['消防'],
    location_name: 'T2消防专项',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '消防专项验收' } },
    batch_type_id: '',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-18 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-19 16:00:00',
    remark: '消防专项已通过（各状态演示·通过）',
    created_by: 'u-sg-01',
    created_at: '2026-07-17 14:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-19 16:00:00',
  },
  {
    id: 'tk-zx-draft',
    task_no: 'ZX-2026-002',
    task_name: '规划专项验收（草稿）',
    project_id: pid,
    wbs_node_id: 'wn-special-planning',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['结构'],
    location_name: 'T2规划核实',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '规划专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '2026-07-22 09:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-22 15:00:00',
    remark: '已驳回存档，可重新申报（同节点另有已通过单·演示一节点一有效单）',
    created_by: 'u-sg-01',
    created_at: '2026-07-22 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-22 15:00:00',
  },
  {
    id: 'tk-zx-pending',
    task_no: 'ZX-2026-003',
    task_name: '人防专项验收',
    project_id: pid,
    wbs_node_id: 'wn-special-cd',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['人防'],
    location_name: 'T2-B1人防',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '人防专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-21 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-21 16:00:00',
    remark: '已驳回存档，可重新申报（同节点另有已通过单）',
    created_by: 'u-sg-01',
    created_at: '2026-07-21 10:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-21 16:00:00',
  },
  {
    id: 'tk-zx-reviewing',
    task_no: 'ZX-2026-004',
    task_name: '节能专项验收',
    project_id: pid,
    wbs_node_id: 'wn-special-energy',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['节能'],
    location_name: 'T2节能专项',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '节能专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-20 11:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-20 18:00:00',
    remark: '已驳回存档，可重新申报（同节点另有已通过单）',
    created_by: 'u-sg-01',
    created_at: '2026-07-19 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-20 18:00:00',
  },
  {
    id: 'tk-zx-fail',
    task_no: 'ZX-2026-005',
    task_name: '特种设备专项验收',
    project_id: pid,
    wbs_node_id: 'wn-special-equip',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['消防'],
    location_name: 'T2客梯/扶梯',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '特种设备专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-16 09:30:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-17 14:00:00',
    remark: '监督检验报告缺项，验收不通过（各状态演示·不通过）',
    created_by: 'u-sg-01',
    created_at: '2026-07-15 10:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-17 14:00:00',
  },
  {
    id: 'tk-zx-rectify',
    task_no: 'ZX-2026-006',
    task_name: '人防专项验收（已驳回）',
    project_id: pid,
    wbs_node_id: 'wn-special-cd',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['人防'],
    location_name: 'T2-B1人防口部',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '人防专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-14 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-15 16:00:00',
    remark: '口部防护密闭门安装偏差，已驳回存档，可重新申报',
    created_by: 'u-sg-01',
    created_at: '2026-07-13 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-15 16:00:00',
  },
  {
    id: 'tk-zx-reinspect',
    task_no: 'ZX-2026-007',
    task_name: '节能专项验收（已驳回）',
    project_id: pid,
    wbs_node_id: 'wn-special-energy',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['节能'],
    location_name: 'T2外围护结构',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '节能专项验收' } },
    batch_type_id: '',
    status: 3,
    result: 2,
    is_hidden_work: 0,
    first_pass_flag: 0,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-12 09:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-18 10:00:00',
    remark: '已驳回存档，可重新申报（原复验路径已废止）',
    created_by: 'u-sg-01',
    created_at: '2026-07-10 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-18 10:00:00',
  },
  {
    id: 'tk-zx-cd-pass',
    task_no: 'ZX-2026-008',
    task_name: '人防专项验收（已通过）',
    project_id: pid,
    wbs_node_id: 'wn-special-cd',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['人防'],
    location_name: 'T2-B1人防主体',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '人防专项验收' } },
    batch_type_id: '',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-08 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-09 17:00:00',
    remark: '人防主体已通过（竣工前置·节点通过样例）',
    created_by: 'u-sg-01',
    created_at: '2026-07-07 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-09 17:00:00',
  },
  {
    id: 'tk-zx-en-pass',
    task_no: 'ZX-2026-009',
    task_name: '节能专项验收（已通过）',
    project_id: pid,
    wbs_node_id: 'wn-special-energy',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['节能'],
    location_name: 'T2屋面保温',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '节能专项验收' } },
    batch_type_id: '',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-06 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-07 16:00:00',
    remark: '屋面保温节能已通过（竣工前置·节点通过样例）',
    created_by: 'u-sg-01',
    created_at: '2026-07-05 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-07 16:00:00',
  },
  {
    id: 'tk-zx-pl-pass',
    task_no: 'ZX-2026-010',
    task_name: '规划专项验收（已通过）',
    project_id: pid,
    wbs_node_id: 'wn-special-planning',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['结构'],
    location_name: 'T2规划核实',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '规划专项验收' } },
    batch_type_id: '',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-04 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-05 15:00:00',
    remark: '规划核实已通过（竣工前置·节点通过样例）',
    created_by: 'u-sg-01',
    created_at: '2026-07-03 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-05 15:00:00',
  },
  {
    id: 'tk-zx-se-pass',
    task_no: 'ZX-2026-011',
    task_name: '特种设备专项验收（已通过）',
    project_id: pid,
    wbs_node_id: 'wn-special-equip',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['消防'],
    location_name: 'T2货梯',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '特种设备专项验收' } },
    batch_type_id: '',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-02 10:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-03 11:00:00',
    remark: '货梯监督检验已通过（竣工前置·节点通过样例）',
    created_by: 'u-sg-01',
    created_at: '2026-07-01 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-03 11:00:00',
  },
  {
    id: 'tk-zx-pending-fire-west',
    task_no: 'ZX-2026-012',
    task_name: '消防专项验收（西翼·待提交）',
    project_id: pid,
    wbs_node_id: 'wn-special-fire-west',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['消防'],
    location_name: 'T2-西翼消防',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '消防专项验收（西翼）' } },
    batch_type_id: '',
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    archive_instance_id: '',
    need_archive: 1,
    related_reject_id: '',
    remark: '待填报提交演示（新专项节点·一节点一有效单）',
    manual_approval_flow: [],
    created_by: 'u-sg-01',
    created_at: '2026-08-10 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-08-10 09:00:00',
  },
  {
    id: 'tk-zx-pending-cd-p2',
    task_no: 'ZX-2026-013',
    task_name: '人防专项验收（二期·待提交）',
    project_id: pid,
    wbs_node_id: 'wn-special-cd-phase2',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['人防'],
    location_name: 'T2-B2人防',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '人防专项验收（二期）' } },
    batch_type_id: '',
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    archive_instance_id: '',
    need_archive: 1,
    related_reject_id: '',
    remark: '待填报提交演示（需归档·电子档案回显）',
    manual_approval_flow: [],
    created_by: 'u-sg-01',
    created_at: '2026-08-10 09:30:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-08-10 09:30:00',
  },
  {
    id: 'tk-zx-pending-energy-sample',
    task_no: 'ZX-2026-014',
    task_name: '节能专项验收（样板区·待提交）',
    project_id: pid,
    wbs_node_id: 'wn-special-energy-sample',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 6,
    specialties: ['节能'],
    location_name: 'T2-样板区节能',
    form_template_id: 'ft-special-fire',
    form_data: { 'ft-special-fire': { 专项名称: '节能专项验收（样板区）' } },
    batch_type_id: '',
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    archive_instance_id: '',
    need_archive: 0,
    related_reject_id: '',
    remark: '待填报提交演示（本次不要求电子档案归档）',
    manual_approval_flow: [],
    created_by: 'u-sg-01',
    created_at: '2026-08-10 10:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-08-10 10:00:00',
  },
  {
    id: 'tk-009',
    task_no: 'YS-2026-009',
    project_id: pid,
    wbs_node_id: 'wn-batch-ready-1',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 1,
    specialties: ['结构'],
    location_name: 'T2-F3-模板-1',
    form_template_id: 'ft-batch-rebar',
    form_data: {},
    batch_type_id: 'bt-rebar',
    status: 2,
    result: 1,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-15 09:00:00',
    reviewer_id: 'u-jl-01',
    finish_time: '2026-07-16 09:00:00',
    remark: '支撑分项解锁的已通过检验批',
    created_by: 'u-sg-01',
    created_at: '2026-07-14 09:00:00',
    updated_by: 'u-jl-01',
    updated_at: '2026-07-16 09:00:00',
  },
  {
    id: 'tk-unit-1',
    task_no: 'DW-2026-001',
    project_id: pid,
    wbs_node_id: 'wn-unit-1',
    plan_id: 'pl-001',
    unplanned_flag: 0,
    parent_task_id: '',
    task_type: 5,
    specialties: ['结构'],
    location_name: 'T2-主体',
    form_template_id: 'ft-unit-record',
    form_data: { 'ft-unit-record': { 单位工程名称: 'T2航站楼主体单位工程' } },
    batch_type_id: '',
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    remark: '实体验收·单位工程样例（待填报）',
    created_by: 'u-sg-01',
    created_at: '2026-07-20 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-20 09:00:00',
  },
  {
    id: 'tk-unit-2',
    task_no: 'DW-2026-002',
    project_id: pid,
    wbs_node_id: 'wn-unit-2',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 5,
    specialties: ['机电'],
    location_name: 'T2-机电',
    form_template_id: 'ft-unit-record',
    form_data: {},
    batch_type_id: '',
    status: 1,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: 1,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: 'u-sg-01',
    submit_time: '2026-07-18 11:00:00',
    reviewer_id: '',
    finish_time: '',
    remark: '实体验收·单位工程样例（审批中）',
    created_by: 'u-sg-01',
    created_at: '2026-07-17 10:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-18 11:00:00',
  },
  {
    id: 'tk-unit-3',
    task_no: 'DW-2026-003',
    project_id: pid,
    wbs_node_id: 'wn-unit-4',
    plan_id: '',
    unplanned_flag: 1,
    parent_task_id: '',
    task_type: 5,
    specialties: ['结构'],
    location_name: 'TUN',
    form_template_id: 'ft-unit-record',
    form_data: {},
    batch_type_id: '',
    // 下级未齐不可报审通过，改为待提交样例
    status: 0,
    result: 0,
    is_hidden_work: 0,
    first_pass_flag: null,
    contractor_org_id: 'org-sg-01',
    supervisor_org_id: 'org-jl-01',
    applicant_id: '',
    submit_time: '',
    reviewer_id: '',
    finish_time: '',
    remark: '实体验收·单位工程样例（待填报；下级分项未齐不可报审）',
    created_by: 'u-sg-01',
    created_at: '2026-07-08 09:00:00',
    updated_by: 'u-sg-01',
    updated_at: '2026-07-08 09:00:00',
  },
])

/**
 * V2.3.1 新增字段归一：历史 seed 默认非草稿；
 * archive_instance_id 与 qmArchive.js 的 ARCHIVE_FORM_INSTANCE seed 对应（一任务一档案文档 Q12）
 */
const ARCHIVE_INSTANCE_BY_TASK = {
  'tk-001': 'afi-001',
  'tk-003': 'afi-003',
  'tk-004': 'afi-004',
  'tk-008': 'afi-008',
  'tk-unit-3': 'afi-u3',
}
inspectionTasks.forEach((t) => {
  if (t.is_draft === undefined) t.is_draft = 0
  if (t.archive_instance_id === undefined) {
    t.archive_instance_id = ARCHIVE_INSTANCE_BY_TASK[t.id] || ''
  }
  if (t.task_name === undefined) {
    t.task_name = wbsNodes.find((n) => n.id === t.wbs_node_id)?.node_name || ''
  }
})

/** TASK_LINK_MATERIAL — 任务关联材料（字段对齐材料设备进场列表） */
export const taskMaterialLinks = reactive([
  {
    id: 'tml-001',
    task_id: 'tk-001',
    material_id: 'JC-2026-001',
    material_name: 'HRB400E 螺纹钢 Φ25',
    material_spec: 'Φ25',
    batch_no: 'PC-20260710-01',
    sample_application_id: 'MS-001',
    supplier: '某钢铁集团',
    brand_name: '某钢',
    quantity_text: '120t',
    use_part: 'T2-F3-A区 轴1~5',
    source_label: '材料',
    submit_time: '2026-07-10 14:20:00',
    link_time: '2026-07-14 09:30:00',
  },
  {
    id: 'tml-002',
    task_id: 'tk-001',
    material_id: 'JC-2026-002',
    material_name: '直螺纹套筒 Φ25',
    material_spec: 'Φ25',
    batch_no: 'PC-20260711-02',
    sample_application_id: '',
    supplier: '某连接技术公司',
    brand_name: '某联',
    quantity_text: '800个',
    use_part: 'T2-F3-A区 轴1~5',
    source_label: '材料',
    submit_time: '2026-07-11 10:00:00',
    link_time: '2026-07-14 09:35:00',
  },
  {
    id: 'tml-003',
    task_id: 'tk-006',
    material_id: 'JC-2026-001',
    material_name: 'HRB400E 螺纹钢 Φ25',
    material_spec: 'Φ25',
    batch_no: 'PC-20260615-03',
    sample_application_id: 'MS-001',
    supplier: '某钢铁集团',
    brand_name: '某钢',
    quantity_text: '80t',
    use_part: 'T2-F2梁板',
    source_label: '材料',
    submit_time: '2026-06-15 09:00:00',
    link_time: '2026-06-19 10:00:00',
  },
])

/** TASK_LINK_SAMPLE — 任务关联定版定样（字段对齐样板报审列表） */
export const taskSampleLinks = reactive([
  {
    id: 'tsl-001',
    task_id: 'tk-unit-1',
    sample_id: 'MS-001',
    sample_name: '外墙真石漆',
    sample_category: '材料设备定样',
    brand_name: '东方雨虹',
    sample_date: '2026-07-08',
    unit_name: 'T3 航站楼',
    use_part: 'T3 航站楼外立面',
    link_time: '2026-07-20 09:10:00',
  },
  {
    id: 'tsl-002',
    task_id: 'tk-008',
    sample_id: 'PS-001',
    sample_name: '清水混凝土柱样板',
    sample_category: '工序样板',
    brand_name: '',
    sample_date: '',
    unit_name: '',
    use_part: 'T2航站楼主体单位工程',
    link_time: '2026-07-17 14:30:00',
  },
])

/** 任务材料关联查询 */
export function getTaskMaterialLinks(task_id) {
  return taskMaterialLinks.filter((l) => l.task_id === task_id)
}

/** 任务定样关联查询 */
export function getTaskSampleLinks(task_id) {
  return taskSampleLinks.filter((l) => l.task_id === task_id)
}

/** TASK_LINK_ASBUILT — 任务关联实模一致验收（字段对齐实模一致列表/详情） */
export const taskAsbuiltLinks = reactive([
  {
    id: 'tal-001',
    task_id: 'tk-001',
    acceptance_id: 'AB-003',
    biz_no: 'AB-202607-003',
    title: '电缆敷设分项实模一致',
    report_names: '实模一致性报告-电缆敷设.pdf',
    node_paths: 'T2航站楼机电单位工程 / 供配电分部 / 电缆敷设分项',
    status: 'approved',
    link_time: '2026-07-22 14:00:00',
  },
])

export function getTaskAsbuiltLinks(task_id) {
  return taskAsbuiltLinks.filter((l) => l.task_id === task_id)
}

/** INSPECTION_ITEM — 产品口径：验评无检查项，种子为空 */
export const inspectionItems = reactive([])

/**
 * RECTIFICATION_ORDER — 验评主路径不支持整改单；种子为空
 */
export const rectificationOrders = reactive([])

export const reinspectRounds = reactive([])

/** APPROVAL_RECORD */
export const approvalRecords = reactive([
  {
    id: 'ar-001',
    task_id: 'tk-003',
    node_name: '监理现场验收',
    action: 3,
    operator_id: 'u-jl-01',
    operator_role: '监理',
    opinion: '受力钢筋连接方式不符合设计，已驳回',
    action_time: '2026-07-12 16:00:00',
  },
])

export const ORG_LABEL = {
  'org-sg-01': '中建某局深圳机场项目部',
  'org-jl-01': '深圳某监理有限公司',
  'org-sg-02': '机场道面施工联合体',
  'org-jl-02': '华南监理咨询公司',
}

/** 目录树节点类型标签色 */
export function wbsNodeTypeTagType(node_type) {
  const map = {
    1: 'danger',
    2: 'warning',
    3: '',
    4: 'success',
    5: 'info',
    6: 'primary',
    7: 'warning',
    8: 'danger',
    9: '',
    10: 'success',
  }
  return map[node_type] ?? 'info'
}

function pushScaffoldNode(partial) {
  const now = nowStr()
  const node = {
    location_code: '',
    batch_type_id: '',
    form_template_ids: [],
    specialties: [],
    is_hidden_work: 0,
    is_critical: 0,
    accept_status: 0,
    batch_scheme_id: '',
    sort_no: 0,
    del_status: 0,
    created_by: 'system',
    created_at: now,
    updated_by: 'system',
    updated_at: now,
    ...partial,
  }
  wbsNodes.push(node)
  return node
}

/** 未逻辑删除的 WBS 节点 */
export function isWbsAlive(n) {
  return Boolean(n) && !Number(n.del_status)
}

/** 节点绑定的表单模板 id 列表（兼容历史单列 form_template_id） */
export function getNodeFormTemplateIds(node) {
  if (!node) return []
  if (Array.isArray(node.form_template_ids)) {
    return node.form_template_ids.map(String).map((s) => s.trim()).filter(Boolean)
  }
  if (node.form_template_id) return [String(node.form_template_id)]
  return []
}

export function primaryFormTemplateId(node) {
  return getNodeFormTemplateIds(node)[0] || ''
}

export function normalizeFormTemplateIdsInput(payload) {
  if (!payload) return []
  if (payload.form_template_ids !== undefined) {
    const raw = payload.form_template_ids
    if (Array.isArray(raw)) return raw.map(String).map((s) => s.trim()).filter(Boolean)
    if (raw == null || raw === '') return []
    return [String(raw)].filter(Boolean)
  }
  if (payload.form_template_id !== undefined && payload.form_template_id !== null) {
    const s = String(payload.form_template_id).trim()
    return s ? [s] : []
  }
  return undefined
}

/**
 * 确保项目具备验评树骨架：竣工验收(根) + 实体工程根 + 专项验收
 * 实体根可挂在竣工下，也可 parent_id 为空（不强制挂竣工）。
 * 并将历史「无父单位工程 / 无父专项节点」挂到对应容器下
 */
export function ensureWbsScaffold(project_id) {
  if (!project_id) return null
  let complete = wbsNodes.find(
    (n) => isWbsAlive(n) && n.project_id === project_id && n.node_type === 8 && !n.parent_id,
  )
  if (!complete) {
    complete = pushScaffoldNode({
      id: `wn-complete-${project_id}`,
      project_id,
      parent_id: '',
      node_type: 8,
      node_name: '项目竣工验收',
      form_template_ids: ['ft-complete'],
      sort_no: 0,
    })
  } else if (!String(complete.node_name || '').includes('竣工')) {
    complete.node_name = '项目竣工验收'
  }

  // 任意父（含空 / 挂竣工）均可；不强制改挂
  let entity = wbsNodes.find(
    (n) => isWbsAlive(n) && n.project_id === project_id && n.node_type === 9,
  )
  if (!entity) {
    entity = pushScaffoldNode({
      id: `wn-entity-${project_id}`,
      project_id,
      parent_id: '',
      node_type: 9,
      node_name: '实体工程验收',
      sort_no: 1,
    })
  }

  let special = wbsNodes.find(
    (n) =>
      isWbsAlive(n) &&
      n.project_id === project_id &&
      n.node_type === 10 &&
      n.parent_id === complete.id,
  )
  if (!special) {
    special = pushScaffoldNode({
      id: `wn-special-root-${project_id}`,
      project_id,
      parent_id: complete.id,
      node_type: 10,
      node_name: '专项验收',
      sort_no: 2,
    })
  }

  wbsNodes
    .filter(
      (n) =>
        isWbsAlive(n) &&
        n.project_id === project_id &&
        n.node_type === 1 &&
        (!n.parent_id || n.parent_id === complete.id),
    )
    .forEach((n) => {
      n.parent_id = entity.id
    })

  wbsNodes
    .filter(
      (n) =>
        isWbsAlive(n) &&
        n.project_id === project_id &&
        n.node_type === 7 &&
        (!n.parent_id || n.parent_id === complete.id),
    )
    .forEach((n) => {
      n.parent_id = special.id
    })

  return { complete, entity, special }
}

export function getCompleteRootNode(project_id) {
  if (!project_id) return null
  ensureWbsScaffold(project_id)
  return (
    wbsNodes.find(
      (n) => isWbsAlive(n) && n.project_id === project_id && n.node_type === 8 && !n.parent_id,
    ) || null
  )
}

export function getEntityRootNode(project_id) {
  if (!project_id) return null
  const s = ensureWbsScaffold(project_id)
  return s?.entity || null
}

export function getSpecialRootNode(project_id) {
  if (!project_id) return null
  const s = ensureWbsScaffold(project_id)
  return s?.special || null
}

/** 验评目录树：竣工验收为根，含实体/专项二级及全部业务节点 */
export function buildWbsTree(project_id) {
  if (project_id) ensureWbsScaffold(project_id)
  let list = wbsNodes.filter(
    (n) => isWbsAlive(n) && WBS_TREE_NODE_TYPES.includes(n.node_type),
  )
  if (project_id) list = list.filter((n) => n.project_id === project_id)
  const map = new Map()
  list.forEach((n) => {
    map.set(n.id, {
      id: n.id,
      label: n.node_name,
      node_type: n.node_type,
      type_label: WBS_TREE_NODE_TYPE_LABEL[n.node_type] || String(n.node_type),
      raw: n,
      children: [],
    })
  })
  const roots = []
  list.forEach((n) => {
    const node = map.get(n.id)
    if (n.parent_id && map.has(n.parent_id)) map.get(n.parent_id).children.push(node)
    else roots.push(node)
  })
  const bySort = (a, b) => (a.raw?.sort_no || 0) - (b.raw?.sort_no || 0)
  const sortDeep = (nodes) => {
    nodes.sort(bySort)
    nodes.forEach((n) => n.children?.length && sortDeep(n.children))
  }
  sortDeep(roots)
  return roots
}

/**
 * 验评目录树 · 树表数据（对齐实体工程分解列表样式）
 * 列：节点名称、节点类型、编码、专业、完整路径、排序值、验收状态
 */
export function buildQmInspectTableTree(projectId) {
  const tree = buildWbsTree(projectId)

  const mapRow = (n, parentPath) => {
    const raw = n.raw || {}
    const nodeName = n.label || raw.node_name || ''
    const fullPath = parentPath ? `${parentPath} / ${nodeName}` : nodeName
    const specialties = getEffectiveSpecialties(raw)
    return {
      id: n.id,
      node_name: nodeName,
      node_type: n.node_type,
      type_label: n.type_label || WBS_TREE_NODE_TYPE_LABEL[n.node_type] || String(n.node_type),
      code: raw.location_code || '',
      specialties: [...specialties],
      full_path: fullPath,
      sort_no: Number(raw.sort_no) || 0,
      accept_status: Number(raw.accept_status) || 0,
      parent_id: raw.parent_id || '',
      project_id: raw.project_id || projectId,
      raw,
      children: (n.children || []).map((c) => mapRow(c, fullPath)),
    }
  }
  return tree.map((n) => mapRow(n, ''))
}

/** 默认展开竣工根与实体/专项分类 */
export function collectQmInspectDefaultExpandKeys(rows) {
  const keys = []
  const walk = (list) => {
    ;(list || []).forEach((r) => {
      if ([8, 9, 10].includes(Number(r.node_type))) keys.push(r.id)
      if (r.children?.length) walk(r.children)
    })
  }
  walk(rows)
  return keys
}

/** 按名称 / 编码 / 路径 / 类型筛选树表（命中父或子均保留分支） */
export function filterQmInspectTableTree(rows, keyword) {
  const kw = String(keyword || '').trim()
  if (!kw) return rows
  const match = (r) =>
    String(r.node_name || '').includes(kw) ||
    String(r.code || '').includes(kw) ||
    String(r.full_path || '').includes(kw) ||
    String(r.type_label || '').includes(kw)

  const walk = (list) => {
    const out = []
    ;(list || []).forEach((r) => {
      const kids = walk(r.children || [])
      if (match(r) || kids.length) {
        out.push({ ...r, children: kids })
      }
    })
    return out
  }
  return walk(rows)
}

export function taskStatusTagType(status) {
  const map = { 0: 'info', 1: 'warning', 2: 'success', 3: 'danger', 4: 'danger', 5: 'warning' }
  return map[status] || 'info'
}

