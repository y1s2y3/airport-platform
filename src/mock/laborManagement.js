...
function getEnteredPersonnel(projectId) {
  const ids =
    !projectId || projectId === 'hq' ? ALL_LABOR_PROJECT_IDS : [projectId]
  return ids.flatMap((id) => getProjectPersonnel(id)).filter(
    (item) => item.entryStatus === REALNAME_ENTRY_STATUS.ENTERED,
  )
}

function aggregateAgeAnalysis(projectId = 'hq') {
  const counts = Object.fromEntries(HQ_AGE_SEGMENTS.map((seg) => [seg.name, 0]))
  getEnteredPersonnel(projectId).forEach((person) => {
...
function aggregateCategoryAnalysis(projectId = 'hq') {
...
function buildAttendanceTrend({ manage = 0, labor = 0 } = {}, endDate = LABOR_HQ_STATS_TODAY) {
  ...
  for (let i = 29; i >= 0; i -= 1) {
    ...
    const manageRate = Math.min(100, rate + 2 + (seed % 3) * 0.3)
    const laborRate = Math.max(80, rate - 1 - (seed % 2) * 0.4)
    const managePresent = Math.round((manage * manageRate) / 100)
    const laborPresent = Math.round((labor * laborRate) / 100)
    const presentCount = managePresent + laborPresent
    const attendanceRate = manage + labor
      ? Number((((presentCount / (manage + labor)) * 100) || 0).toFixed(1))
...
function buildWarningTrend(projectId = 'hq', endDate = LABOR_HQ_STATS_TODAY) {
  const warnings = getProjectWarnings(projectId)
  const end = new Date(endDate)
  const trend = []
  for (let i = 29; i >= 0; i -= 1) {
    const date = new Date(end)
    date.setDate(end.getDate() - i)
    const dateStr = date.toISOString().slice(0, 10)
    const newCount = warnings.filter((w) => String(w.triggeredAt || '').startsWith(dateStr)).length
    // 演示：当日未处置 = 触发日≤当日且状态为待处理，加少量种子波动便于出图
    const pendingBase = warnings.filter(
      (w) =>
        w.status === '待处理' &&
        String(w.triggeredAt || '').slice(0, 10) <= dateStr,
    ).length
    const seed = Number(dateStr.replace(/-/g, '')) % 5
    const pendingCount = Math.max(0, pendingBase - (i % 3) + (seed % 2))
    trend.push({
      date: dateStr,
      label: dateStr.slice(5),
      newCount,
...
function buildDashboardPayload({ realname, warningStats, warnings, personStats }) {
  ...
  return {
    summary: {
      ...
      todayManageRate: '-',
      todayWarningCount: 0,
      pendingWarningCount: warningStats.pending,
    },
...
export function getLaborDashboardData(projectId) {
  const scopeId = !projectId || projectId === 'hq' ? 'hq' : projectId
  const realname =
...
export const attendancePersonList = [
  ...
  { name: '赵磊', company: '广东建工', workType: '特种-电工', attendanceDays: 20, totalHours: 176, avgHours: 8.8, lateCount: 0, earlyLeaveCount: 0, absentCount: 2, overtimeHours: 16, attendanceRate: '90.9%' },
  { name: '刘洋', company: '深圳市政', workType: '特种-焊工', attendanceDays: 22, totalHours: 202, avgHours: 9.2, lateCount: 0, earlyLeaveCount: 0, absentCount: 0, overtimeHours: 20, attendanceRate: '100%' },
  ...
  { name: '周杰', company: '中建三局', workType: '特种-架子工', attendanceDays: 19, totalHours: 168, avgHours: 8.8, lateCount: 3, earlyLeaveCount: 2, absentCount: 3, overtimeHours: 10, attendanceRate: '86.4%' },
]
...
export function buildDashboardKpiSummary(projectId, today = LABOR_HQ_STATS_TODAY) {
  if (!projectId || projectId === 'hq') {
    ...
    return {
      ...
      labor: s.labor,
      special: s.special,
      todayAttendanceRate: formatDashboardRate(s.todayAttendanceRate),
      todayManageRate: formatDashboardRate(s.todayManageRate),
      todayWarningCount: s.todayWarningCount,
      pendingWarningCount: s.pendingWarningCount,
    }
  }
  const row = buildHqRealNameSupervisionStatsByProject(today).find((item) => item.project_id === projectId)
  if (!row || row.demoEmpty) {
    return {
      total: 0,
      manage: 0,
      labor: 0,
  ...
  return {
    ...
    labor: row.labor,
    special: row.special,
    todayAttendanceRate: formatDashboardRate(row.todayAttendanceRate),
    todayManageRate: formatDashboardRate(row.todayManageRate),
    todayWarningCount: row.todayWarningCount,
    pendingWarningCount: row.pendingWarningCount,
  }
}
