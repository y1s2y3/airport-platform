/**
 * GitHub Pages 发布后递增版本末位 x（v{BASE}.x）
...
if (!baseMatch || !patchMatch) {
  console.error('Cannot find APP_VERSION_BASE / APP_VERSION_PATCH in appVersion.js')
  process.exit(1)
}
...
console.log(`Bumped version: v${base}.${prev} → ${version}${highlights.length ? ' (with notes)' : ''}`)
